...
from otp_u2f.utils import websafe_cbor_decode, websafe_cbor_encode
...
def test_webauthn_register_complete(webauthn):
    ...
    response = {
        'id': credential_id,
        'rawId': credential_id,
        'response': websafe_cbor_decode('_aF3hn1zLS8_CqAzDW7sSirmsXnvXVRQQAAAAAAAAAAAAAAAAAAAAAAAAAAAEBbDIQ3s6clvoqFBL80OfigZuJU9K4jGYrCj7vuv_ycbYEqGjJCVi-4be5LxZrrzxiL-1k0UbuZeFFoLeVS1YKfpQECAyYgASFYIGX54GU6pZBsdbVEw6B7sGCrtKUaHmu62JTMBLd_U64_IlggERQvKwWtfZX8mvREWzv1mrTh2tsLvHlcCCH4247nZpM')  # noqa
    }
    ...
    assert websafe_encode(credential.credential_id) == credential_id
    assert credential.public_key == websafe_cbor_decode(public_key)
